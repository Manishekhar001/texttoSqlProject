    ...
    async def generate_answer(
        ...
        try:
            ...
            embedding, embedding_usage = (
                await self.embedding_service.generate_embeddings([question])
            )
            ...
            combined_usage = {
                "embedding_tokens": (
                    embedding_usage.get("total_tokens", 0) if embedding_usage else None
                ),
                "llm_prompt_tokens": (
                    llm_usage.get("prompt_tokens", 0) if llm_usage else 0
                ),
                "llm_completion_tokens": (
                    llm_usage.get("completion_tokens", 0) if llm_usage else 0
                ),
                "total_tokens": (
            ...
            if self.query_cache_service and self.query_cache_service.enabled:
                ...
                self.query_cache_service.set(
                    cache_key, result, ttl=ttl, cache_type="rag"
                )
                logger.info(
